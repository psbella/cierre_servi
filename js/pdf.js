...
function exportarPDF() {
    ...
    pdf.text('CIERRE DE TURNO - SERVI v2', 105, 20, { align: 'center' });
    ...
    y += 6;
    pdf.text(`Total Reloj: ${formatearPesos(r.totalReloj)}`, 20, y);
    y += 6;
    pdf.text(`Relevo: - ${formatearPesos(r.relevo)}`, 20, y);
    y += 6;
    pdf.text(`Total Recaudacion: ${formatearPesos(r.totalRecaudacion)}`, 20, y);
    y += 6;
    ...
    pdf.text(`Firma Ticket: ${formatearPesos(r.firmaTicket)}`, 120, y);
    y += 6;
    ...
    pdf.text(`Combustible: - ${formatearPesos(r.combustible)}`, 20, y);
    y += 5;
    pdf.text(`Subtotal: ${formatearPesos(r.subtotal)}`, 20, y);
    y += 5;
    pdf.text(`50%: ${formatearPesos(r.base50)}`, 20, y);
    y += 5;
    pdf.text(`Frecuencia: + ${formatearPesos(r.frecuencia)}`, 20, y);
    y += 5;
    pdf.text(`Sub total: ${formatearPesos(r.subTotal)}`, 20, y);
    y += 5;
    pdf.text(`Tarjeta/QR: - ${formatearPesos(r.tarjetaQr)}`, 20, y);
    y += 5;
    pdf.text(`Voucher: - ${formatearPesos(r.voucher)}`, 20, y);
    y += 5;
    pdf.text(`Firma Ticket: - ${formatearPesos(r.firmaTicket)}`, 20, y);
    y += 5;
    pdf.text(`Cuenta Corriente: - ${formatearPesos(r.cuentaCorriente)}`, 20, y);
    y += 5;
    ...
    pdf.text('Formula aplicada: (Total Reloj - Relevo - Combustible) / 2 = Base 50%', 20, y);
    y += 4;
    pdf.text('Titular = Base + Frecuencia - (Tarjeta/QR + Voucher + Firma Ticket + Cta Cte + Gastos)', 20, y);
    y += 4;
